...
> 提出ブランチ：`feature/14-publishing`（PRのbase：`develop`）
...
- [パフォーマンスと最適化](./13-performance.md) を完了し、計測→改善の型がある
...
1. 上記の演習を完了する
2. GitHub で `feature/14-publishing` ブランチを作成し、PRを作成
...
次は [Unit 7: 総合演習（ガイド）](./07-unit7-guide.md) に進み、総合演習の進め方（チケット分割）を確認しましょう。
