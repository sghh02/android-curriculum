...
> 提出ブランチ：`feature/22-workmanager`（PRのbase：`develop`）
...
- [画面サイズ対応（スマホ/タブレット）](./21-adaptive-ui.md) を完了し、発展要件の進め方がわかる
...
次は [ComposeとViewの相互運用](./23-compose-view-interop.md) に進み、ComposeとViewの相互運用を押さえましょう。
