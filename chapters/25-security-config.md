...
> 提出ブランチ：`feature/25-security-config`（PRのbase：`develop`）
...
- [アクセシビリティと多言語対応](./24-accessibility-i18n.md) を完了し、発展要件の流れがわかる
...
1. 上記の演習を完了する
2. GitHub で `feature/25-security-config` ブランチを作成し、PRを作成
...
次は [Unit 11: 模擬案件（チケット演習）（ガイド）](./11-unit11-guide.md) に進み、模擬案件（チケット演習）の進め方を確認しましょう。
