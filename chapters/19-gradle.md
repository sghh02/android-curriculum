...
> 提出ブランチ：`feature/19-gradle`（PRのbase：`develop`）

実務の現場では「機能開発」だけでなく、**依存関係追加・更新・ビルドエラー対応**が普通に降ってきます。
...
【やりたいこと】
（例：依存関係を追加したい / ビルドエラーを直したい / Version Catalogを整えたい）

【今の状態】
- エラー全文：
- 関連するbuild.gradle(.kts)/settings：
- 最近入れた依存：
...
- Releaseでだけ壊れる事故を、どう早めに潰す？
- 自分のプロジェクトの「境界（UI/feature/core/data）」は整理できそう？
