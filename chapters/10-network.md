...
> 提出ブランチ：`feature/10-network`（PRのbase：`develop`）
...
【やりたいこと】
（例：APIを呼びたい / エラーをUIに出したい / リトライを入れたい）
...
- [ ] 取得中（Loading）/失敗（Error）/成功（Success）の表示を揃える
- [ ] 失敗時のリトライ導線（ボタン）を作り、二重リクエストを防ぐ
- [ ] エラーを「ユーザー向けメッセージ」と「調査用ログ」に分ける方針を決める
...
- リトライは、UXとしてどうあるべき？（自動/手動/回数制限）
- ネットワークのログは、何を出して何を出さない？
