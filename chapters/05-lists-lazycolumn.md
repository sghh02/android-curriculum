...
> 提出ブランチ：`feature/05-lists-lazycolumn`（PRのbase：`main`）
...
- [ ] シンプルメモアプリの一覧を `LazyColumn` にし、アイテム表示をComposableに切り出す
- [ ] `key` を設定し、削除/追加しても表示が崩れないことを確認する
- [ ] 空状態（0件）と読み込み中（Loading）のUIを用意する
...
- リストの「責務の分割」はどこでやると読みやすい？（Item/Section/Screen）
- シンプルメモの一覧で、次に改善したいUXは何？（空状態/検索/並び替え）
